...
| :--- | :--- | :--- |
| **Surface** | Layered Whites (#FFFFFF to #F8F9FF) | Deep Navies/Grays (#0A1128 to #121A2F) |
| **Shadows** | Soft, multi-layered "Linear" shadows | Ambient, glowing elevation "Apple-style" |
| **Glass** | White-based backdrop blur (60% op) | Navy-based backdrop blur (40% op) |
| **Contrast** | WCAG AA compliant on white | High-legibility on deep dark |
...
### 4.1 Behavior
- **Modes:** Collapsed (Icons only), Expanded (Full labels), Mobile (Drawer).
...
2. **Workspace** (Buildings, Units, Meters, SIM Cards)
3. **Operations** (Readings, Validation)
4. **Financial** (Billing, Invoices, Payments, Collections, Ledger)
5. **Analytics** (Executive, Financial, Revenue, Consumption)
6. **Monitoring** (Live Grid, Device Health, Alerts)
7. **Center** (Import, Migration, Jasper Reports)
8. **Admin** (Users, Audit, Permissions, System Settings)
...
1. **Global Header:** Search, Notifications, Localizer.
...
- **Borders:** 1px Solid, subtle contrast (Outline-variant).

---

## 7. POPUP & DIALOG PHILOSOPHY
Small actions = Zero navigation.
- **Animation:** 200ms "Scale-in" from center.
- **Backdrop:** 12px blur + semi-transparent overlay.
- **Focus:** Auto-focus first input; ESC to close.
...
### 8.1 Data Tables
- **High Density:** Optimized for complex data viewing.
- **Features:** Sticky headers, column pinning, bulk selection, row-level animations (fade-in).
- **Interaction:** Hover state highlights entire row with brand-tint.

### 8.2 Charts
- **Palette:** Colors must map to the semantic logic (e.g., Water = Cyan, Electric = Yellow-Orange).
- **Polish:** Tooltips must use the "Glass" card style.
- **Types:** Support for Area (Trends), Gauge (Health), and Radar (Multi-parameter comparison).
...
- **Hover Lift:** Cards elevate 4px with increased shadow spread.
...
- **Icons:** Directional icons (arrows, chevrons) must flip.
