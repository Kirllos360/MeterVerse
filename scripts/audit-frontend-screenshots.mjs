...
for (const p of adminPages) {
  try {
    await page.goto("http://localhost:3535/admin/" + p, { waitUntil: "networkidle", timeout: 15000 }).catch(() => {})
    await page.waitForTimeout(1500)
    await page.screenshot({ path: dir + "/admin-" + p + ".png", fullPage: false })
    console.log("âœ… admin-" + p)
  } catch (e) { console.log("âŒ admin-" + p) }
}

// ============ USER/ROOT PAGES ============
await page.goto("http://localhost:3535/", { waitUntil: "networkidle", timeout: 30000 }).catch(() => {})
await page.waitForTimeout(3000)
await page.screenshot({ path: dir + "/root-home.png", fullPage: false })
console.log("âœ… root-home")
