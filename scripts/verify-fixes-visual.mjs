...
await page.goto("http://localhost:7400/admin", { timeout: 60000, waitUntil: "domcontentloaded" }).catch(() => {})
await new Promise(r => setTimeout(r, 5000))
await page.screenshot({ path: dir + "/01-admin-root.png" })
console.log("✅ 01-admin-root — Shortcut Menu, ConnectionHeader")

// Check if "Shortcut Menu" text exists
const hasShortcut = await page.evaluate(() => document.body.innerText.includes("Shortcut Menu"))
console.log("  Shortcut Menu text: " + (hasShortcut ? "✅ VISIBLE" : "❌ MISSING"))
...
console.log("  Connection signal: " + (hasSignal ? "✅ PRESENT" : "❌ MISSING"))

// Check user dropdown
await page.goto("http://localhost:7400/admin/login", { timeout: 30000, waitUntil: "domcontentloaded" }).catch(() => {})
await new Promise(r => setTimeout(r, 3000))
await page.screenshot({ path: dir + "/02-login-page.png" })
console.log("✅ 02-login-page")
...
await browser.close()
console.log("\n✅ All screenshots captured in " + dir)
