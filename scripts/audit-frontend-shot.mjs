...
const takeScreenshot = async (url, name) => {
  try {
    ...
    console.log("âŒ " + name + ": " + e.message.substring(0, 60))
  }
}

// Admin pages
await takeScreenshot("http://localhost:3535/admin", "admin-root")
...
for (const p of pages) {
  await takeScreenshot("http://localhost:3535/admin/" + p, "admin-" + p)
}

// Root page
await takeScreenshot("http://localhost:3535/", "root-home")

await browser.close()
console.log("ALL DONE â€” screenshots in " + dir)
