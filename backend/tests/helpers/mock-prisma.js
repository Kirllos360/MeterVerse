import { vi } from 'vitest';

function createMockModel() {
  return {
    findMany: vi.fn(),
    findUnique: vi.fn(),
    findFirst: vi.fn(),
    create: vi.fn(),
    update: vi.fn(),
    updateMany: vi.fn(),
    delete: vi.fn(),
    deleteMany: vi.fn(),
    upsert: vi.fn(),
    count: vi.fn(),
    aggregate: vi.fn(),
    groupBy: vi.fn(),
  };
}

function createMockDelegate() {
  return {
    findMany: vi.fn(),
    findUnique: vi.fn(),
    findFirst: vi.fn(),
    create: vi.fn(),
    update: vi.fn(),
    updateMany: vi.fn(),
    delete: vi.fn(),
    deleteMany: vi.fn(),
    upsert: vi.fn(),
    count: vi.fn(),
  };
}

export const prisma = {
  customer: createMockModel(),
  meter: createMockModel(),
  reading: createMockModel(),
  invoice: createMockModel(),
  payment: createMockModel(),
  outboxEvent: createMockModel(),
  eventDelivery: createMockModel(),
  eventDeadLetter: createMockModel(),
  idempotencyRecord: createMockModel(),
  serviceIdentity: createMockModel(),
  serviceCredential: createMockModel(),
  user: createMockModel(),
  role: createMockModel(),
  permission: createMockModel(),
  auditEntry: createMockModel(),
  organization: createMockModel(),
  area: createMockModel(),
  project: createMockModel(),
  tariff: createMockModel(),
  tariffRate: createMockModel(),
  tariffTier: createMockModel(),
  chargeRule: createMockModel(),
  settlement: createMockModel(),
  invoiceSettlement: createMockModel(),
  discountRule: createMockModel(),
  contract: createMockModel(),
  contractTerm: createMockModel(),
  contractAmendment: createMockModel(),
  billCycle: createMockModel(),
  billRun: createMockModel(),
  billRunHistory: createMockModel(),
  invoiceItem: createMockModel(),
  invoiceTax: createMockModel(),
  meterAssignment: createMockModel(),
  meterAssignmentHistory: createMockModel(),
  meterEvent: createMockModel(),
  measurementPoint: createMockModel(),
  measPointResType: createMockModel(),
  resultType: createMockModel(),
  result: createMockModel(),
  resultM: createMockModel(),
  quantity: createMockModel(),
  notification: createMockModel(),
  notificationTemplate: createMockModel(),
  emailLog: createMockModel(),
  smsLog: createMockModel(),
  activityStream: createMockModel(),
  session: createMockModel(),
  permissionOnRole: createMockModel(),
  userRole: createMockModel(),
  userPreference: createMockModel(),
  kpiDefinition: createMockModel(),
  kpiSnapshot: createMockModel(),
  reportDefinition: createMockModel(),
  alertRule: createMockModel(),
  alert: createMockModel(),
  validationRule: createMockModel(),
  validationResult: createMockModel(),
  workflowState: createMockModel(),
  workflowTransition: createMockModel(),
  storedFile: createMockModel(),
  importJob: createMockModel(),
  exportJob: createMockModel(),
  collectionCase: createMockModel(),
  collectionAction: createMockModel(),
  promiseToPay: createMockModel(),
  paymentGateway: createMockModel(),
  paymentTransaction: createMockModel(),
  gatewayLog: createMockModel(),
  customerGroup: createMockModel(),
  groupMember: createMockModel(),
  groupPricing: createMockModel(),
  sla: createMockModel(),
  slaBreach: createMockModel(),
  slaEscalation: createMockModel(),
  groupSla: createMockModel(),
  escalationPolicy: createMockModel(),
  escalationStep: createMockModel(),
  customerContact: createMockModel(),
  notificationPreference: createMockModel(),
  apiKey: createMockModel(),
  systemSetting: createMockModel(),
  featureFlag: createMockModel(),
  backup: createMockModel(),
  cacheEntry: createMockModel(),
  queueJob: createMockModel(),
  scheduledTask: createMockModel(),
  license: createMockModel(),
  brandingConfig: createMockModel(),
  webhook: createMockModel(),
  scheduledReport: createMockModel(),
  exportLog: createMockModel(),
  chargeOverride: createMockModel(),
  mprt: createMockModel(),
  mprtfk: createMockModel(),
  sIMCard: createMockModel(),
  sIMAssignment: createMockModel(),
  task: createMockModel(),
  zone: createMockModel(),
  unit: createMockModel(),
  serviceConnection: createMockModel(),
  customerLedgerEntry: createMockModel(),
  meterType: createMockModel(),
  governanceStandard: createMockModel(),
  governancePolicy: createMockModel(),
  governanceDecision: createMockModel(),
  architectureDecisionRecord: createMockModel(),
  governanceException: createMockModel(),
  governanceWaiver: createMockModel(),
  technicalDebtItem: createMockModel(),
  businessRisk: createMockModel(),
  complianceObligation: createMockModel(),
  governanceAuditFinding: createMockModel(),
  tenant: createMockModel(),
  tenantSetting: createMockModel(),
  subscriptionPlan: createMockModel(),
  tenantSubscription: createMockModel(),
  usageMeter: createMockModel(),
  environmentProfile: createMockModel(),
  workflowDefinition: createMockModel(),
  workflowVersion: createMockModel(),
  workflowNode: createMockModel(),
  workflowEdge: createMockModel(),
  workflowInstance: createMockModel(),
  workflowTask: createMockModel(),
  approvalRequest: createMockModel(),
  approvalDecision: createMockModel(),
  account: createMockModel(),
  journalEntry: createMockModel(),
  journalLineItem: createMockModel(),
  generalLedgerEntry: createMockModel(),
  financialPeriod: createMockModel(),
  financialEvent: createMockModel(),
  accountMapping: createMockModel(),
  auditTrailEntry: createMockModel(),
  revenueRule: createMockModel(),
  revenueLeakageFinding: createMockModel(),
  revenueInvestigation: createMockModel(),
  tariffVersion: createMockModel(),
  tariffVersionRate: createMockModel(),
  tariffVersionTier: createMockModel(),
  tariffToUSchedule: createMockModel(),
  tariffDemandRate: createMockModel(),
  tariffFixedCharge: createMockModel(),
  tariffTax: createMockModel(),
  tariffChangeLog: createMockModel(),
  customerTariff: createMockModel(),
  customerRiskProfile: createMockModel(),
  dunningRule: createMockModel(),
  installmentPlan: createMockModel(),
  planInstallment: createMockModel(),
  dispute: createMockModel(),
  provisionRule: createMockModel(),
  badDebtProvision: createMockModel(),
  writeOffRequest: createMockModel(),
  financialSnapshot: createMockModel(),
  budget: createMockModel(),
  budgetVsActual: createMockModel(),
  financialRatio: createMockModel(),
  reportSchedule: createMockModel(),
  financialNote: createMockModel(),
  iFRSMapping: createMockModel(),
  segmentPerformance: createMockModel(),
  financialForecast: createMockModel(),
  financialScenario: createMockModel(),
  monteCarloResult: createMockModel(),
  businessHealthScore: createMockModel(),
  executiveInsight: createMockModel(),
  aiModelVersion: createMockModel(),
  aiRecommendationLog: createMockModel(),
  document: createMockModel(),
  documentVersion: createMockModel(),
  documentCategory: createMockModel(),
  documentTag: createMockModel(),
  documentDocumentTag: createMockModel(),
  retentionPolicy: createMockModel(),
  documentApproval: createMockModel(),
  documentComment: createMockModel(),
  conversation: createMockModel(),
  message: createMockModel(),
  deliveryAttempt: createMockModel(),
  notificationPreference: createMockModel(),
  customerPreference: createMockModel(),
  delegatedAccess: createMockModel(),
  serviceRequest: createMockModel(),
  serviceRequestMessage: createMockModel(),
  customerDocument: createMockModel(),
  ticket: createMockModel(),
  $transaction: vi.fn((callback) => callback(prisma)),
  $disconnect: vi.fn(),
  $on: vi.fn(),
  $use: vi.fn(),
  $extends: vi.fn(),
};

export function resetPrismaMocks() {
  for (const model of Object.values(prisma)) {
    if (typeof model === 'object' && model !== null) {
      for (const method of Object.values(model)) {
        if (typeof method === 'function' && 'mockReset' in method) {
          method.mockReset();
        }
      }
    }
  }
}
