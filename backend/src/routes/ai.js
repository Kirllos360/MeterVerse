...
router.post("/operator", requirePermission("ai.*"), async (req, res, next) => {
  try {
...
router.post("/billing-assistant", requirePermission("ai.*"), async (req, res, next) => {
  try {
...
router.post("/reading-validator", requirePermission("ai.*"), async (req, res, next) => {
  try {
...
router.post("/leak-detection", requirePermission("ai.*"), async (req, res, next) => {
  try {
    const { meterId } = z.object({ meterId: z.string().uuid() }).parse(req.body)
...
router.get("/forecasting", requirePermission("ai.*"), async (req, res, next) => {
  try {
    const entityType = (req.query.entityType) || "consumption"
...
router.post("/root-cause", requirePermission("ai.*"), async (req, res, next) => {
  try {
    const { invoiceId } = z.object({ invoiceId: z.string().uuid() }).parse(req.body)
...
router.post("/report-builder", requirePermission("ai.*"), async (req, res, next) => {
  try {
...
router.post("/sql-assistant", requirePermission("ai.*"), async (req, res, next) => {
  try {
    const { query } = z.object({ query: z.string().min(1).max(500) }).parse(req.body)
...
router.post("/workflow-generator", requirePermission("ai.*"), async (req, res, next) => {
  try {
