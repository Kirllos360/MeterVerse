...
router.get("/projects", requirePermission("admin.*"), async (req, res, next) => {
  try {
    ...
    const [projects, total] = await Promise.all([
      prisma.project.findMany({ where, skip: (page - 1) * limit, take: limit, orderBy: { createdAt: "desc" }, include: { organization: { select: { id: true, name: true } }, _count: { select: { zones: true } } } }),
...
router.get("/projects/:id", requirePermission("admin.*"), async (req, res, next) => {
  try {
    const project = await prisma.project.findUnique({ where: { id: req.params.id }, include: { organization: { select: { id: true, name: true } }, _count: { select: { zones: true } } } })
...
router.put("/projects/:id", requirePermission("admin.*"), async (req, res, next) => {
  try {
    const existing = await prisma.project.findUnique({ where: { id: req.params.id } })
    if (!existing) return res.status(404).json({ error: "Project not found" })
    const data = z.object({ name: z.string().min(1).optional(), description: z.string().optional(), status: z.enum(["active", "inactive"]).optional(), taxEnabled: z.boolean().optional(), taxRate: z.number().min(0).max(1).optional(), waterDifferenceMode: z.enum(["billable", "report_only"]).optional(), readingThreshold: z.number().positive().optional(), paymentTermsDays: z.number().int().min(0).max(365).optional() }).parse(req.body)
    const project = await prisma.project.update({ where: { id: req.params.id }, data })
    auditLog(req, "admin.project.updated", { projectId: project.id })
    res.json({ project })
  } catch (err) { next(err) }
})

router.delete("/projects/:id", requirePermission("admin.*"), async (req, res, next) => {
  try {
    ...
    auditLog(req, "admin.project.archived", { projectId: project.id })
    res.json({ project })
  } catch (err) { next(err) }
})

router.post("/projects/:id/restore", requirePermission("admin.*"), async (req, res, next) => {
  try {
    const existing = await prisma.project.findUnique({ where: { id: req.params.id } })
    if (!existing) return res.status(404).json({ error: "Project not found" })
    const project = await prisma.project.update({ where: { id: req.params.id }, data: { status: "active", archivedAt: null } })
    auditLog(req, "admin.project.restored", { projectId: project.id })
    res.json({ project })
  } catch (err) { next(err) }
})
