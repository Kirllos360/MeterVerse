...
router.post("/:modelName/:id/delete", requireRole("admin", "super_admin"), auditMiddleware(req, "entity.action"), async (req, res, next) => {
  try { const item = await softDelete(req.params.modelName, req.params.id, req.user.sub); res.json({ success: true, item }) }
  catch (err) { next(err) }
})

router.post("/:modelName/:id/restore", requireRole("admin", "super_admin"), auditMiddleware(req, "entity.action"), async (req, res, next) => {
  try { const item = await toggleArchive(req.params.modelName, req.params.id, req.user.sub); res.json({ success: true, item }) }
  catch (err) { next(err) }
})

router.post("/:modelName/bulk-update", requireRole("admin", "super_admin"), auditMiddleware(req, "entity.action"), async (req, res, next) => {
  try {
...
router.post("/:modelName/bulk-delete", requireRole("admin", "super_admin"), auditMiddleware(req, "entity.action"), async (req, res, next) => {
  try {
...
router.post("/:modelName/import", requireRole("admin", "super_admin"), auditMiddleware(req, "entity.action"), async (req, res, next) => {
  try {
...
router.post("/undo/:auditEntryId", requireRole("admin", "super_admin"), auditMiddleware(req, "entity.action"), async (req, res, next) => {
  try { const result = await undoAction(req.params.auditEntryId, req.user.sub); res.json(result) }
  catch (err) { next(err) }
})

router.post("/:modelName/:id/submit-approval", requireRole("admin", "super_admin"), auditMiddleware(req, "entity.action"), async (req, res, next) => {
...
router.post("/:modelName/:id/approve", requireRole("admin", "super_admin"), auditMiddleware(req, "entity.action"), async (req, res, next) => {
...
router.post("/:modelName/:id/reject", requireRole("super_admin"), auditMiddleware(req, "entity.action"), async (req, res, next) => {
