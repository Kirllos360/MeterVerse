# P43 — Wave 2 Implementation Execution Plan (Billing & Finance Foundation)

**Version:** 1.0.0  
**Status:** READ ONLY — EXECUTION PLANNING — NO CODE / NO SCHEMA CHANGES  
**Date:** 2026-08-01  
...
|---|---|---|
| C12 Identity & Zero Trust | ✅ Complete (Wave 1) | auth-engine, security middleware (31 tests), JWT/RBAC/MFA/area-scope |
| C19 Platform Admin & DevSecOps | ✅ Complete (Wave 1) | 5 CI workflows, config-center, health, Symbiot isolation |
| C20 Quality Gates | ✅ Complete (Wave 1) | contract+integration CI gate, coverage thresholds 46/36/48/43 |
| C21 Governance | ✅ Complete (Wave 1) | governance registries (10 models, 30 routes), RBAC `governance.*` |
| Existing billing modules | ✅ Present | `routes/billing.js` (BillRun CRUD, invoice approve/reject/cancel), `services/billing-engine.js` |
| Accounting backend | ✅ Present | `routes/accounting.js` (accounts, journal entries, GL, trial balance, periods), 5 models live |
| Customer/Meter/Invoice/Payment models | ✅ Present | Customer, Meter, Invoice, Payment, InvoiceItem, InvoiceTax, Tariff, TariffRate, TariffTier |
| Workflow engine | ✅ Present | `workflow-engine.js` (customer/invoice/meter state machines), WorkflowState/Transition persisted |
| C22 seeds | ⚠️ Partial | Organization, Country, License exist; Tenant/Subscription/Plan/UsageMeter missing |
...
C23 Workflow Foundation
        ↓
C13 Financial Integration (billing → GL)
        ↓
...
### Step 2.1 — C22 Tenant Foundation
...
### Step 2.2 — C23 Workflow Foundation
...
### Step 2.3 — C13 Financial Integration (billing → GL)

| Aspect | Detail |
|---|---|
| **Objective** | FinancialEvent bridge: invoice issue/payment → journal → GL auto-posting |
...
| **API impact** | `services/posting-engine.js` + hooks in invoices.js/payments.js/billing.js (≤15 lines each) |
...
### Step 2.4 — C13 Revenue Intelligence
...
### Step 2.6 — C13 Collection Intelligence
...
### Step 2.7 — C13 Financial Reporting
...
### Step 2.8 — C13 Financial AI

| Aspect | Detail |
|---|---|
| **Objective** | Forecasting, scenario/Monte Carlo, health score, executive recommendations |
| **Dependencies** | Steps 2.3-2.7 (data), C18 AI (later wave — use rule-based + C12-W07 patterns now) |
...
| B-03 | C22 → C23 | WorkflowDefinition, WorkflowVersion, WorkflowNode, WorkflowEdge, WorkflowInstance, WorkflowTask, ApprovalRequest, ApprovalDecision | Additive | Down script |
...
- `tenantId` additions nullable → backfill from Organization → required at later wave.
- Invoice/Payment `periodId`/`journalEntryId` nullable → backfilled by posting engine.
...
|---|---|---|---|---|
| Revenue leakage detection | Flag/assign findings | ≥0.7 auto-flag; <0.7 review | Correction actions, write-off | Full |
| Collection prioritization | Score/rank | ≥0.8 auto-rank; <0.8 review | Dunning actions, PTP changes | Full |
| Revenue forecast | Read-only projection | ≥0.6 present; <0.6 suppress | None (read-only) | Full |
| Invoice anomaly | Flag anomalies | ≥0.7 auto-flag; <0.7 review | Block/issue decisions | Full |
| Financial classification | Suggest account codes | ≥0.9 auto-suggest; 0.7-0.9 review; <0.7 manual | Journal posting | Full |
| Scenario/forecast | Read-only simulation | ≥0.6 present | Any production action | Full |
...
| M3: C13 Financial Integration | 8 tasks | Invoice→GL auto-posting verified, 75 unit tests | +2% |
...
**Wave 2 total: ~62 tasks, ~623 unit tests + integration/contract/Playwright/security/regression. Estimated coverage: 12% → 22%.**
...
```
□ C22: Tenant CRUD + isolation + subscription lifecycle live; tenant-scope enforced on new routes.
□ C23: Workflow definitions/versions/instances + approval engine live above existing state machines.
□ C13: Invoice/payment → FinancialEvent → Journal → GL auto-posting verified (debit=credit, period lock, reversal).
□ C13: 15 revenue assurance rules active; findings + investigation workflow + dashboard.
□ C13: Versioned tariff engine (lifecycle, ToU/tiered/demand) + simulation.
□ C13: Dunning/PTP/installment collection intelligence + workbench dashboards.
□ C13: P&L, Balance Sheet, Cash Flow, Budget vs Actual from GL.
□ C13: Forecasts, scenarios, Monte Carlo, business health score; no autonomous financial mutation.
□ All steps pass C20 gates (tsc 0, vitest, coverage, contract, Playwright, security, regression).
□ All migrations additive; down scripts verified; validated on clean staging.
□ P40 execution tracker updated; Wave 2 coverage 12% → 22%.
```
...
```
□ Wave 2 certified per P42-style gate (GO) before Wave 3.
□ All Wave-2 unit tests (623) + integration (93) + contract (64) + Playwright (28) pass.
□ No regression in Wave 1 (C12/C19/C20/C21) or live C01-C10 connectivity.
□ Billing→GL chain fully traced: invoice → event → journal → GL → report.
□ No autonomous financial mutation; all AI financial actions governed.
□ Repository pushed to Kirllos360/MeterVerse after each milestone.
```

---

*This is an execution planning artifact only. No code, schema change, or implementation is included.*
*P43 — Wave 2 Implementation Execution Plan (Billing & Finance Foundation). READ ONLY.*
