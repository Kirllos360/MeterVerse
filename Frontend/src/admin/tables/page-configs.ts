...
export const pageConfigs: Record<string, PageConfig> = {
  ...
  "reports-v2": {
    id: "reports-v2", title: "Reports v2 (Async)", description: "Async report generation with progress tracking",
    apiEndpoint: "/api/reports/exports",
    serverSide: true, statusField,
    transform: (d: any) => (d.exports || []).map((e: any) => ({
      id: e.id, name: e.name || e.reportName || "Export",
      format: e.format || e.type || "PDF",
      progress: e.progress ?? (e.status === "completed" ? 100 : e.status === "running" ? 50 : 0),
      status: e.status || "queued", createdAt: e.createdAt || "",
    })),
    columns: [
      { id: "name", header: "Export", accessor: r => r.name, type: "avatar", width: 220 },
      { id: "format", header: "Format", accessor: r => r.format, type: "badge", width: 100 },
      { id: "progress", header: "Progress", accessor: r => `${r.progress}%`, width: 100 },
      { id: "status", header: "Status", accessor: r => r.status, type: "status", width: 100 },
      { id: "createdAt", header: "Created", accessor: r => r.createdAt, type: "date", width: 110 },
    ],
    fields: defFields([]),
