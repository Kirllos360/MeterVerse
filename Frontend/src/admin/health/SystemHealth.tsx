...
export function SystemHealth() {
  ...
  return (
    <div className="rounded-xl border overflow-hidden" style={{ borderColor: "var(--border-default, #E5E5E5)" }}>
      <div className="px-4 py-3 border-b" style={{ borderColor: "var(--border-default, #E5E5E5)", backgroundColor: "var(--surface-raised, #FFFFFF)" }}>
        <h2 className="text-sm font-semibold" style={{ color: "var(--text-primary, #0A0A0A)" }}>Service Health</h2>
      </div>
      <div style={{ backgroundColor: "var(--surface-base, #FAFAFA)" }}>
        {services.map((s) => (
          <div key={s.name} className="flex items-center justify-between px-4 py-2.5 border-b text-xs" style={{ borderColor: "var(--border-default, #E5E5E5)" }}>
            <div className="flex items-center gap-2">
              <span className="w-2 h-2 rounded-full" style={{ backgroundColor: COLORS[s.status] }} />
              <span style={{ color: "var(--text-primary, #0A0A0A)" }}>{s.name}</span>
            </div>
            <div className="flex items-center gap-3">
              <span style={{ color: COLORS[s.status] }}>{s.status}</span>
              <span style={{ color: "var(--text-tertiary, #A3A3A3)" }}>{s.latency}</span>
            </div>
          </div>
        ))}
      </div>
