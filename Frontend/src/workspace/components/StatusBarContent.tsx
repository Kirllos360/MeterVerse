    ...
    <footer className="flex items-center h-10 px-4 text-sm font-semibold select-none" style={{ color: "var(--text-tertiary)" }}>
      ...
      <div className="flex items-center gap-3 flex-1 min-w-0">
        ...
        <AnimatePresence mode="wait">
          {showQuote ? (
            <motion.span key={`quote-${quoteIndex}`} initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }} transition={{ duration: 0.3 }} className="truncate text-[10px]" style={{ color: "rgba(var(--brand-rgb), 0.6)" }}>
      ...
      <div className="flex items-center gap-3 shrink-0">
        ...
        <motion.span className="text-[10px] tabular-nums" style={{ color: "rgba(var(--brand-rgb), 0.5)" }}
