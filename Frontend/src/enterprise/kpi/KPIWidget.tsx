...
function TrendIcon({ trend }: { trend: "up" | "down" | "flat" }) {
  const colors = { up: "#059669", down: "#DC2626", flat: "#6B7280" }
  return (
...
export function KPIWidget({ title, value, unit, trend, trendValue, color = "var(--brand-primary, #00BFA5)", formatter, delay = 0 }: KPIWidgetProps) {
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-2">
        <span className="text-[11px] font-medium" style={{ color: "var(--text-tertiary, #A3A3A3)" }}>{title}</span>
        {trend && (
          <div className="flex items-center gap-1">
            <TrendIcon trend={trend} />
            {trendValue && <span className="text-[10px]" style={{ color: trend === "up" ? "#059669" : trend === "down" ? "#DC2626" : "#6B7280" }}>{trendValue}</span>}
          </div>
        )}
      </div>
      <div className="flex items-baseline gap-1">
        ...
        {unit && <span className="text-xs" style={{ color: "var(--text-tertiary, #A3A3A3)" }}>{unit}</span>}
      </div>
      {/* Bottom glow bar */}
