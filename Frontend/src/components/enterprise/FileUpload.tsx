...
export function FileUpload({ accept, maxSize = 10485760, onUpload }: FileUploadProps) {
  ...
  return (
    <div className="space-y-3">
      <div {...getRootProps()} className={`border-2 border-dashed rounded-xl p-6 text-center cursor-pointer transition-colors ${isDragActive ? "border-[var(--brand)]" : "border-[var(--border-default)]"}`} style={{ backgroundColor: isDragActive ? "rgba(var(--brand-rgb), 0.03)" : "var(--surface-raised)" }}>
      ...
      <AnimatePresence>
        {files.map((f, i) => (
          <motion.div key={i} initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, height: 0 }}
            ...
            {f.status === "uploading" && <div className="w-20 h-1.5 rounded-full overflow-hidden" style={{ backgroundColor: "var(--surface-sunken)" }}><motion.div className="h-full rounded-full" style={{ backgroundColor: "var(--brand)" }} animate={{ width: ["0%", "100%"] }} transition={{ duration: 2 }} /></div>}
            ...
            {f.status === "pending" && (
              <button onClick={() => setFiles((prev) => prev.filter((_, idx) => idx !== i))} aria-label="Remove file" className="text-xs" style={{ color: "var(--text-tertiary)" }}>
      ...
      {files.some((f) => f.status === "pending") && (
        <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
          ...
          className="w-full py-2 rounded-lg text-sm font-medium text-white" style={{ backgroundColor: isUploading ? "rgba(var(--brand-rgb), 0.5)" : "var(--brand)" }}>
