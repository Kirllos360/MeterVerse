  ...
  return (
    <form onSubmit={onSubmit} className='space-y-2 sm:space-y-3' aria-label='Reply composer'>
      ...
      <div className='border-border/40 bg-background/80 flex items-end gap-2 rounded-2xl border p-3 backdrop-blur sm:gap-3 sm:rounded-3xl sm:p-4'>
          ...
          <div className='mt-2 flex flex-wrap gap-2 sm:mt-3 sm:gap-2'>
            {quickReplies.map((reply) => (
              <button
                ...
                className='border-border/50 bg-background/70 text-muted-foreground hover:border-primary/40 hover:text-foreground focus-visible:ring-primary/40 focus-visible:ring-offset-background rounded-full border px-3 py-1 text-[0.65rem] transition focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:outline-none sm:px-3 sm:py-1 sm:text-xs'
              >
                {reply}
              </button>
            ))}
        ...
        <div className='flex shrink-0 flex-col items-end gap-2 sm:w-24 sm:gap-2'>
          <input
            ref={fileInputRef}
