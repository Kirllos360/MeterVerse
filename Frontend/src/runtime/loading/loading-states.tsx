...
export function SkeletonRuntime({ message = "Loading..." }: LoadingStateProps) {
  return (
    <div className="flex flex-col gap-4 p-6">
      {[1, 2, 3].map((i) => (
        <div key={i} className="h-8 rounded-lg animate-pulse" style={{ backgroundColor: "var(--surface-sunken)", width: `${100 - i * 20}%` }} />
      ))}
...
export function EmptyRuntime({ message = "No data yet", description }: { message?: string; description?: string }) {
  return (
    <motion.div variants={variants.fadeIn} initial="initial" animate="animate" className="flex flex-col items-center justify-center py-16 px-6 text-center">
      <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1" className="mb-4" style={{ color: "var(--text-tertiary)" }}>
...
export function OfflineRuntime({ message = "You are offline", onRetry }: { message?: string; onRetry?: () => void }) {
  return (
    <motion.div variants={variants.fadeIn} initial="initial" animate="animate" className="flex flex-col items-center justify-center py-16 px-6 text-center">
      <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="mb-4" style={{ color: "var(--status-error)" }}>
...
export function PermissionRuntime({ message = "Access denied" }: { message?: string }) {
  return (
    <motion.div variants={variants.fadeIn} initial="initial" animate="animate" className="flex flex-col items-center justify-center py-16 px-6 text-center">
      <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="mb-4" style={{ color: "var(--status-pending)" }}>
        <rect x="3" y="11" width="18" height="11" rx="2" />
        <path d="M7 11V7a5 5 0 0110 0v4" />
      </svg>
      <p className="text-base font-medium" style={{ color: "var(--text-primary)" }}>{message}</p>
      <p className="text-sm mt-1" style={{ color: "var(--text-tertiary)" }}>You don't have permission to access this area.</p>
    </motion.div>
  )
}
