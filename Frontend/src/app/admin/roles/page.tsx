    ...
    <div className="p-6 space-y-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        {["Super Admin", "Admin", "Manager", "Operator", "Viewer", "Custom"].map((role) => (
          <div key={role} className="p-4 rounded-xl border" style={{ backgroundColor: "#0A0A0A", borderColor: "#1A1A1A" }}>
            <div className="flex items-center justify-between mb-3">
              <span className="text-sm font-medium text-white">{role}</span>
              <span className="px-2 py-0.5 rounded text-[10px] font-medium" style={{ backgroundColor: "rgba(239,68,68,0.15)", color: "#EF4444" }}>{role === "Super Admin" ? "Full Access" : "Limited"}</span>
            </div>
