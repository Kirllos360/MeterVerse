  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border p-6" style={{ backgroundColor: "var(--surface-raised)", borderColor: "var(--border-default)" }}>
        ...
        {!loading && tab === 0 && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>{table(["Name", "Code", "Status", "Manager", "Phone"], CENTERS.map(c => [c.name, c.code, <span className="px-2 py-0.5 rounded-full text-xs" style={{ backgroundColor: c.status === "Active" ? "rgba(34,197,94,0.1)" : "rgba(156,163,175,0.1)", color: c.status === "Active" ? "#22c55e" : "#6b7280" }}>{c.status}</span>, c.manager, c.phone]))}</motion.div>}
