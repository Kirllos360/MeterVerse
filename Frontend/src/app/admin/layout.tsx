...
const waveBorder = { scale: [1, 1.03, 1], opacity: [0.4, 0.8, 0.4], transition: { repeat: Infinity, duration: 2.5, ease: "easeInOut" } }
  ...
  return (
    <div style={{ ...themeVars, backgroundColor: "var(--surface-base)" }} dir={lang === "ar" ? "rtl" : "ltr"} className="h-screen w-screen overflow-hidden flex flex-col">
      ...
      <div className="fixed inset-0 pointer-events-none z-0" style={{
        background: isLight
          ? `radial-gradient(800px circle at ${mousePos.x * 100}% ${mousePos.y * 100}%, rgba(220,38,38,0.03) 0%, transparent 60%)`
          : `radial-gradient(800px circle at ${mousePos.x * 100}% ${mousePos.y * 100}%, rgba(220,38,38,0.06) 0%, transparent 60%)`,
        ...
        <div className="flex gap-0.5 overflow-x-auto py-1 scrollbar-none justify-center">
          {SYSTEM_TABS.map(tab => (
            <button key={tab.id} onClick={() => setSystemTab(tab.id)}
              ...
              {systemTab === tab.id && <motion.div layoutId="sysTabBg" className="absolute inset-0 rounded-lg" style={{ backgroundColor: "rgba(220,38,38,0.08)" }} />}
              ...
              {tab.label}
              {systemTab === tab.id && <motion.div layoutId="sysTabDot" className="w-1 h-1 rounded-full" style={{ backgroundColor: "var(--brand)" }} />}
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="relative z-10 flex flex-1 min-h-0 overflow-hidden p-2 gap-2">
                    ...
                    return (
                      <motion.button key={item.id} onClick={() => setActivePage(item.id as any)}
                        ...
                        <span className="relative z-10 flex items-center gap-3" style={{
                          ...
                          {!sidebarCollapsed && (
                            <span className="truncate font-bold text-[11px] tracking-wide">{t(lang, item.label)}</span>
                          )}
                        </span>
                      </motion.button>
                    )
        ...
        <div className="flex-1 flex flex-col min-w-0 gap-2">
          ...
          {/* PAGE CONTENT — animated border in dark mode */}
      ...
      <div className="relative z-10 shrink-0 flex items-center" style={{ height: 56, backgroundColor: "var(--surface-topbar)", borderTop: "1px solid var(--border-default)" }}>
        <div className="flex items-center justify-between w-full px-5 text-[12px] font-bold tracking-wide" style={{ color: "var(--text-tertiary)" }}>
          <div className="flex items-center gap-3">
            <motion.div animate={waveAnim} className="w-2.5 h-2.5 rounded-full shrink-0" style={{ backgroundColor: "var(--brand)" }} />
          ...
          <div className="flex items-center gap-3">
            <span className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full" style={{ backgroundColor: "var(--brand)" }} /> All Systems Normal</span>
            <span className="w-px h-3" style={{ backgroundColor: "var(--border-default)" }} />
            <motion.span animate={waveAnim} className="w-2 h-2 rounded-full" style={{ backgroundColor: "var(--brand)" }} />
          </div>
        </div>
      </div>
    </div>
