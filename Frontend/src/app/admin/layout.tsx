...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const themeVars = {
    ...
    "--surface-base": isLight ? "#FFFFFF" : "#1A1A1A",
    "--surface-topbar": isLight ? "#FFFFFF" : "#1A1A1A",
    "--surface-raised": isLight ? "#FFFFFF" : "#222222",
    "--sidebar-background": isLight ? "#F5F5F5" : "#0D0D0D",
    "--border-default": isLight ? "rgba(0,0,0,0.08)" : "rgba(255,255,255,0.08)",
    "--text-primary": isLight ? "#1A1A1A" : "#FFFFFF",
    "--text-secondary": isLight ? "rgba(0,0,0,0.6)" : "rgba(255,255,255,0.6)",
    "--text-tertiary": isLight ? "rgba(0,0,0,0.35)" : "rgba(255,255,255,0.35)",
    "--toolbar-bg": isLight ? "rgba(255,255,255,0.9)" : "rgba(26,26,26,0.9)",
    "--toolbar-border": isLight ? "rgba(0,0,0,0.08)" : "rgba(255,255,255,0.08)",
    "--toolbar-text": isLight ? "#1A1A1A" : "#FFFFFF",
  ...
  return (
    <div style={{ ...themeVars, backgroundColor: "var(--surface-base)" }} dir={lang === "ar" ? "rtl" : "ltr"} className="h-screen w-screen overflow-hidden flex flex-col">
      ...
      <div className="flex flex-1 min-h-0 overflow-hidden">
        {/* Sidebar */}
        <motion.div animate={{ width: sidebarCollapsed ? 52 : 200 }} className="shrink-0 flex flex-col overflow-hidden" style={{ backgroundColor: "var(--sidebar-background)", borderRight: "1px solid var(--border-default)" }}>
                  ...
                  {group.items.map(item => {
                    ...
                    return (
                        <motion.button key={item.id} onClick={() => setActivePage(item.id as any)}
                          className="flex items-center w-full rounded text-xs outline-none relative"
                          style={{ padding: sidebarCollapsed ? "9px 7px" : "6px 10px", color: isActive ? "var(--brand)" : "var(--text-secondary)" }}>
                          {isActive && <motion.div layoutId="activeNav" className="absolute left-0 top-0.5 bottom-0.5 w-0.5 rounded" style={{ backgroundColor: "var(--brand)" }} />}
                          <motion.div className="absolute inset-0 rounded" initial={false} animate={{ backgroundColor: isActive ? "rgba(var(--brand-rgb),0.08)" : "transparent" }} transition={{ duration: 0.15 }} />
                          {!isActive && <div className="absolute inset-0 rounded hover:bg-black/[0.03] dark:hover:bg-white/[0.04] transition-colors" />}
                        <span className="relative z-10 flex items-center gap-2.5">
                          ...
                          {!sidebarCollapsed && <span className="truncate text-xs font-medium">{t(lang, item.label)}</span>}
                        </span>
                      </motion.button>
                    )
                  })}
          ...
          <motion.button onClick={() => setSidebarCollapsed(!sidebarCollapsed)} className="flex items-center justify-center py-2 shrink-0" style={{ borderTop: "1px solid var(--border-default)", color: "var(--text-tertiary)" }}>
