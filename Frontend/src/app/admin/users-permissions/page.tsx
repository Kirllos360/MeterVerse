      ...
      <div className="rounded-2xl border p-6" style={{ backgroundColor: "var(--surface-raised)", borderColor: "var(--border-default)" }}>
        ...
        {!loading && tab === 2 && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-lg space-y-4">
            ...
            <div><label htmlFor="lbl-mfa-enforcement" className="text-xs font-semibold block mb-1" style={{ color: "var(--text-secondary)" }}>MFA Enforcement</label><select id="lbl-mfa-enforcement" value={mfaEnforcement} onChange={e => setMfaEnforcement(e.target.value)} className="w-full rounded-xl border px-3 py-2 text-xs outline-none" style={{ backgroundColor: "var(--surface-topbar)", borderColor: "var(--border-default)", color: "var(--text-primary)" }}><option>All Users</option><option>Admins Only</option><option>Disabled</option></select></div>
        ...
        {!loading && tab === 4 && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
          {users.length === 0 ? <p className="text-xs py-4" style={{ color: "var(--text-secondary)" }}>No user profiles</p> : table(["Name", "Email", "Role", "Status"], users.map((u: any) => [u.name, u.email, u.role, <span key={u.id ?? u.name ?? u} className="px-2 py-0.5 rounded-full text-xs" style={{ backgroundColor: u.status === "active" ? "rgba(220,38,38,0.1)" : "rgba(156,163,175,0.1)", color: u.status === "active" ? "#DC2626" : "#6b7280" }}>{u.status}</span>]))}
        </motion.div>}
