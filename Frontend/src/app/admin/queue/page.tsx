...
const STATUS_COLORS: Record<string,string> = {pending:"#F59E0B",running:"#3B82F6",completed:"#22C55E",failed:"#EF4444"}
          ...
          <tbody>
            ...
            : jobs.map(j => (
              <tr key={j.id}>
                ...
                <td className="px-4 py-3 text-sm">{j.status==="failed"&&<button onClick={()=>retry(j.id)} className="text-[10px] px-2 py-1 rounded font-medium" style={{backgroundColor:"rgba(245,158,11,0.1)",color:"#F59E0B"}}>Retry</button>}</td>
              </tr>
            ))}
          </tbody>
        </table>
