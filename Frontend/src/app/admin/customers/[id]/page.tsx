...
export default function CustomerDetailPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        setData(res.customer as DetailData)
  ...
  if (!data) return <div className="p-6 text-center"><h2 className="text-xl font-semibold">Customer not found</h2><Button onClick={() => router.back()} className="mt-4">Go back</Button></div>

  return (
    <ErrorBoundary>
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-bold">Customer {data.id?.toString().slice(0, 8)}</h1></div>
