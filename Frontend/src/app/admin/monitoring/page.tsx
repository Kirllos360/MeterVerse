    ...
    <div className="p-6 space-y-4">
      ...
      {tab === "dashboard" && <>
          ...
          {deep?.checks?.map((c: any,i: number) => (
            <div key={i} className="rounded-xl border p-4" style={{borderColor:"var(--admin-border)",backgroundColor:"var(--admin-surface)"}}>
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="px-2 py-0.5 rounded text-[10px] font-medium" style={{backgroundColor:c.status==="healthy"?"rgba(34,197,94,0.1)":"rgba(245,158,11,0.1)",color:c.status==="healthy"?"#22C55E":"#F59E0B"}}>{c.status}</span>
              </div>
        ...
        <div className="rounded-xl border p-4" style={{borderColor:"var(--admin-border)",backgroundColor:"var(--admin-surface)"}}>
          <div className="text-xs mb-2" style={{color:"rgba(255,255,255,0.4)"}}>Overall Status: <span style={{color:deep?.status==="healthy"?"#22C55E":"#F59E0B",fontWeight:600}}>{deep?.status?.toUpperCase()}</span></div>
      ...
      {tab === "performance" && <>
        <div className="grid grid-cols-4 gap-3">
          {[
            {l:"Total Readings",v:perf?.summary?.totalReadings||0,c:"#3B82F6"},
            {l:"Validation Rate",v:`${perf?.summary?.validationRate||0}%`,c:"#22C55E"},
            {l:"Total Invoices",v:perf?.summary?.totalInvoices||0,c:"#F59E0B"},
            {l:"Revenue",v:`EGP ${(perf?.summary?.totalRevenue||0).toLocaleString()}`,c:"#22C55E"},
          ].map(s => (
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            {l:"Today's Readings",v:perf?.today?.readings||0,c:"#3B82F6"},
            {l:"Today's Invoices",v:perf?.today?.invoices||0,c:"#22C55E"},
            {l:"Throughput (30d avg)",v:`${perf?.throughput?.readingsPerDay||0}/day`,c:"#F59E0B"},
          ].map(s => (
            <div key={s.l} className="rounded-xl border p-4" style={{borderColor:"var(--admin-border)",backgroundColor:"var(--admin-surface)"}}>
              <div className="text-xs" style={{color:"rgba(255,255,255,0.4)"}}>{s.l}</div>
              <div className="text-lg font-bold mt-1 tabular-nums" style={{color:s.c}}>{s.v}</div>
      ...
      {tab === "audit" && <>
        <div className="grid grid-cols-3 gap-3">
          {[
            {l:"Total Entries",v:audit?.stats?.total||0,c:"#3B82F6"},
            {l:"Failures",v:audit?.stats?.failures||0,c:audit?.stats?.failures>0?"#EF4444":"#22C55E"},
            {l:"Today",v:audit?.stats?.today||0,c:"#F59E0B"},
          ].map(s => (
            <div key={s.l} className="rounded-xl border p-4" style={{borderColor:"var(--admin-border)",backgroundColor:"var(--admin-surface)"}}>
              <div className="text-xs" style={{color:"rgba(255,255,255,0.4)"}}>{s.l}</div>
              <div className="text-lg font-bold mt-1 tabular-nums" style={{color:s.c}}>{s.v}</div>
      ...
      {tab === "analytics" && <>
        <div className="grid grid-cols-4 gap-3">
          {[
            {l:"New Customers (30d)",v:analytics?.growth?.customers||0,c:"#3B82F6"},
            {l:"New Meters (30d)",v:analytics?.growth?.meters||0,c:"#22C55E"},
            {l:"Readings (30d)",v:analytics?.growth?.readings||0,c:"#F59E0B"},
