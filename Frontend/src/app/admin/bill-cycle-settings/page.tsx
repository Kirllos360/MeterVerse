    ...
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border p-6" style={{ backgroundColor: "var(--surface-raised)", borderColor: "var(--border-default)" }}>
        ...
        {!loading && tab === 2 && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
          {table(["Project", "Meters Read", "Verified", "Failed", "Status"], VERIFICATION.map(v => [v.project, String(v.metersRead), String(v.verified), String(v.failed), <span className="px-2 py-0.5 rounded-full text-xs" style={{ backgroundColor: v.status === "Verified" ? "rgba(34,197,94,0.1)" : v.status === "Pending" ? "rgba(245,158,11,0.1)" : "rgba(239,68,68,0.1)", color: v.status === "Verified" ? "#22c55e" : v.status === "Pending" ? "#f59e0b" : "#ef4444" }}>{v.status}</span>]))}
        </motion.div>}
