...
export default function AdminLoginPage() {
  ...
  if (success) {
    return (
      <div className="flex min-h-screen items-center justify-center" style={{ backgroundColor: "#064E3B" }}>
        <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ type: "spring", stiffness: 200, damping: 20 }}
          ...
          <motion.div animate={{ rotate: [0, 10, -10, 0] }} transition={{ duration: 0.5 }}
            className="w-16 h-16 mx-auto mb-4 rounded-2xl flex items-center justify-center" style={{ backgroundColor: "#00BFA5" }}>
  ...
  return (
    <div className="flex min-h-screen" style={{ backgroundColor: "#FAFAFA" }}>
      {/* Left Panel */}
      <div className="hidden lg:flex w-1/2 flex-col justify-center px-16 relative overflow-hidden" style={{ backgroundColor: "#064E3B" }}>
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
          <motion.div whileHover={{ rotate: [0, -5, 5, 0] }} className="w-14 h-14 rounded-2xl flex items-center justify-center mb-6" style={{ backgroundColor: "#00BFA5" }}>
          ...
          {["Role-based Access Control", "Multi-tenant Architecture", "Real-time Monitoring", "Advanced Analytics"].map((f, i) => (
            <motion.div key={f} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 + i * 0.1 }}
              ...
              <motion.svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#00BFA5" strokeWidth="2.5"
      ...
      <div className="flex-1 flex items-center justify-center p-8">
        <motion.div key={mode} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.4 }}
          ...
          <div className="lg:hidden flex items-center gap-3 mb-8">
            <motion.div whileHover={{ rotate: [0, -5, 5, 0] }} className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ backgroundColor: "#00BFA5" }}>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {mode === "signup" && (
              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} transition={{ duration: 0.3 }}>
                ...
                <input type="text" value={name} onChange={(e) => setName(e.target.value)}
                  className="w-full px-3 py-2.5 rounded-lg border text-sm outline-none transition-colors focus:border-[#00BFA5]"
            ...
            {mode === "login" && (
              <div className="flex items-center justify-between">
                ...
                <button type="button" className="text-xs font-medium" style={{ color: "#00BFA5" }}>Forgot password?</button>
              </div>
            )}

            {(error || localError) && (
              <motion.div initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }}
                className="p-3 rounded-lg text-xs" style={{ backgroundColor: "rgba(220,38,38,0.08)", color: "#DC2626" }}>
                {localError || error}
            ...
            <motion.button type="submit" disabled={isLoading}
              ...
              style={{ backgroundColor: isLoading ? "rgba(0,191,165,0.5)" : "#00BFA5" }}>
              {isLoading ? (
          ...
          <div className="mt-4 text-center">
            <button onClick={() => { setMode(mode === "login" ? "signup" : "login"); setLocalError(null) }}
              className="text-xs font-medium hover:underline" style={{ color: "#00BFA5" }}>
