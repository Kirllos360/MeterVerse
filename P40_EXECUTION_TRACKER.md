...
| OBS-075 | P55 Enterprise Forensic Gap Analysis — CERTIFIED | Zero-trust audit. **Fixed**: ~25 operational pages wired in pageMap but NOT in admin nav (Accounting, Collections, Alerts, Sim, Zones, Units, Service Connections, Meter Assignments, Notifications, Roles, Permissions, Scheduler, Queue, Tasks, Webhooks, Integrations, Connectivity, Runtime, Backup) → added to AdminLayout ALL_NAV_ITEMS (commit 3c2f2847). **Verified**: 4 services live (3535/3131/3030/3003), 17 core pages render 0 page/console/4xx errors, functional trace create-customer 201→persisted→reload, portal :3030 user version. Full exam: backend 286, frontend tsc 0 + vitest 44, production build. **Concerns (non-blocking)**: health-scores profilesTracked=0; ~13 pageMap-only pages (Security/SMS/SMTP/Storage/Sync/Themes/Business/Workflows/Services/Sessions/API-Keys/Cache/Connectivity-Center); mock data on Upload; dev pre-auth 401; db-push migration history (2 applied vs 15 dirs). Reports: `docs/reviews/P55_{FORENSIC_GAP_ANALYSIS,CHATGPT_HANDOVER}.md`. | High | Platform safe to continue. Next: Wave 4 (C15→C26→C17) per P40. |
| OBS-076 | P56 Reconciliation + AI Alignment Prompts | **Critical fixes (zero-trust)**: (1) Portal :3030 was rendering the ADMIN console — client root gate used server-only PORTAL_MODE; fixed with NEXT_PUBLIC_PORTAL_MODE in page.tsx + portal launchers + package.json portal scripts (commit a0cfdf2c); verified portal :3030 = user version, no admin modules. (2) Admin :3535/ now serves console at root (200, no redirect). (3) Portal BE restarted with PORTAL_MODE=1 → admin routes 404. (4) UTF-8 BOM removed from package.json files (broke next parsing). **Deliverable**: full-context brain prompts for ChatGPT + Kimi to independently decide open architecture: `docs/reviews/P56_CHATGPT_BRAIN_PROMPT.md` + `P56_KIMI_CONTEXT_PROMPT.md` — Q1 DB separation (data-tenancy is THE blocker), Q2 shared login (profile-aware backend, decided), Q3 two kirllos super-admin accounts, Q4 sign-out routing gap, Q5 area/project/user role-permission wiring, Q6 tooling (prisma migrate, CI E2E). | High | Awaiting AI decisions on Q1-Q6 before implementing login/accounts/wiring. Next: Wave 4 (C15→C26→C17). |
| OBS-077 | P57 Enterprise Zero-Trust Master Recovery — CERTIFIED | **Fixed**: 48 test/script files referenced stale ports (7400/3002/3001) → migrated to admin :3535 / API :3131 (commit dbb0700b). Zero stale ports in code. This un-skipped previously-skipped backend tests → **backend 292 all pass** (was 274+18 skip). **Verified**: 4 services live+healthy (3535/3131/3030/3003, portal admin-gate 404), 12/12 core pages render 0 errors (Playwright walk incl. P55 Operations pages), functional wiring create→persist→reload, frontend tsc 0 + vitest 44, production build succeeds. **Known dev-only fragility**: Turbopack dev-cache corruption (CSS/routes.d.ts) when both FEs run on shared source — cleaned caches; recommend launcher auto-clean step. Report: `docs/reviews/P57_MASTER_RECOVERY_CERTIFICATION.md`. | High | Platform verified end-to-end GO. Awaiting ChatGPT/Kimi Q1-Q6 decisions (DB separation, login, kirllos accounts, wiring). Next: Wave 4 (C15→C26→C17). |
| OBS-078 | P57 Permanent admin/portal separation + visual identity | **CRITICAL FIX**: Admin :3535 and Portal :3030 BOTH showed the SAME admin data (portal FE started without PORTAL_MODE env). **Permanent fix (commit 2b27c274)**: profile derived from runtime PORT, not env — page.tsx window.location.port==='3030'→portal (else admin); layout.tsx request host/port header → 3030=portal green else admin red. Two ports can NEVER show same profile again. **Verified (Playwright)**: :3535=admin red ADMINISTRATION; :3030=portal green DASHBOARD user nav only (no admin modules). **Visual identity (commit 3d66d335)**: dark=black gutter + dark-gray sectors(#1A1A1E)+red+white text+light-red hover; light=white gutter+off-white sectors(#F2F2F5)+red+black text+light-red hover; auto-theme by clock; chart greens→red; breadcrumb removed; operations-guide stale ports fixed. **Open**: DB tenancy (P56 Q1), shared login/sign-out, kirllos accounts, area/project/user wiring, semantic-green decision, Wave 4. Reports: `docs/reviews/P57_{MASTER_RECOVERY_CERTIFICATION,PORT_AND_FRONTEND_PLANNING_REFERENCE,CHATGPT_PLAN_ADJUSTMENT}.md`. | High | Permanent separation verified. Awaiting ChatGPT plan adjustment (P57_CHATGPT_PLAN_ADJUSTMENT.md). Next: Wave 4 (C15→C26→C17). |
